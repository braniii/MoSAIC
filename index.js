...
"ref":"mosaic",
"url":0,
"doc":"                MoSAIC Motion   Docs \u2022  Features \u2022  Installation \u2022  Usage    Molecular Systems Automated Identification of Collective Motion MoSAIC is a new algorithm for correlation analysis which auto-matically detects collective motion in MD simulation data, identifies uncorrelated features as noise and hence provides a detailed picture of the key coordinates driving a conformational change in a biomolecular system. It is based on a community detection algorithm which is used to bring a correlation matrix in a block-diagonal form. The method was published in: > G. Diez, D. Nagel, and G. Stock,  Molecular Systems Automated Identification > of Collective Motion (MoSAIC motion) , in preparation We kindly ask you to cite these articles if you use this software package for published works.  Features - Intuitive usage via [module]( module -inside-a-python-script) and via [CI]( ci -usage-directly-from-the-command-line) - Sklearn-style API for fast integration into a Python workflow - No magic, only single parameter - Extensive [documentation](https: moldyn.github.io/feature_selection) and long discussion in publication  Installation So far the package is only published to [PyPI](https: pypi.org). Soon, it will be added [conda-forge](https: conda-forge.org/), as well. For installing it to within a python environment simple call:   python3 -m pip install  upgrade feature_selection   or for the latest dev version    via ssh key python3 -m pip install git+ssh: git@github.com/moldyn/feature_selection.git  , or via password-based login python3 -m pip install git+https: github.com/moldyn/feature_selection.git    Shell Completion Using the  bash ,  zsh or  fish shell click provides an easy way to provide shell completion, checkout the [docs](https: click.palletsprojects.com/en/8.0.x/shell-completion). In the case of bash you need to add following line to your  ~/.bashrc   eval \"$(_MOSAIC_COMPLETE=bash_source MoSAIC)\"    Usage In general one can call the module directly by its entry point  $ MoSAIC or by calling the module  $ python -m mosaic . The latter method is preferred to ensure using the desired python environment. For enabling the shell completion, the entry point needs to be used.  CI - Usage Directly from the Command Line The module brings a rich CI using [click](https: click.palletsprojects.com). Each module and submodule contains a detailed help. It can be called by   $ python -m mosaic Usage: python -m mosaic [OPTIONS] COMMAND [ARGS] . MoSAIC motion v0.1.0 Molecular systems automated identification of collective motion, is a correlation based feature selection framework for MD data. Copyright (c) 2021, Georg Diez and Daniel Nagel Options:  help Show this message and exit. Commands: clustering Clustering similarity matrix of coordinates. similarity Creating similarity matrix of coordinates. umap Embedd similarity matrix with UMAP.   For more details on the submodule one needs to specify one of the three commands. A simple workflow example for clustering the input file  input_file using correlation and Leiden with CPM and the default resolution parameter:    creating correlation matrix $ python -m mosaic similarity -i input_file -o output_similarity -metric correlation -v MoSAIC SIMILARITY  ~ Initialize similarity class  ~ Load file input_file  ~ Fit input  ~ Store similarity matrix in output_similarity  clustering with CPM and default resolution parameter  the latter needs to be fine-tuned to each matrix $ python -m mosaic clustering -i output_similarity -o output_clustering  plot -v MoSAIC CLUSTERING  ~ Initialize clustering class  ~ Load file output_similarity  ~ Fit input  ~ Store output  ~ Plot matrix   This will generate the similarity matrix stored in  output_similarity , the plotted result in  output_clustering.matrix.pdf , the raw data of the matrix in  output_clustering.matrix and a file containing in each row the indices of a cluster.  Module - Inside a Python Script   import mosaic  Load file  X is np.ndarray of shape (n_samples, n_features) sim = mosaic.Similarity( metric='correlation',  or 'NMI', 'GY', 'JSD' ) sim.fit(X)  Cluster matrix clust = mosaic.Clustering( mode='CPM',  or 'modularity ) clust.fit(sim.matrix_) clusters = clust.clusters_ clusterd_X = clust.matrix_  .  "
},
{
"ref":"mosaic.Clustering",
"url":0,
"doc":"Class for clustering a correlation matrix. Parameters      mode : str, default='CPM' the mode which determines the quality function optimized by the Leiden algorithm. - 'CPM': will use the constant Potts model on the full, weighted graph - 'modularity': will use modularity on a knn-graph weighted : bool, default=True, If True, the underlying graph has weighted edges. Otherwise, the graph is constructed using the adjacency matrix. n_iterations : int, default=None, Number of iterations to run the Leiden algorithm. None means that the algorithm runs until no further improvement is achieved. n_neighbors : int, default=None, This parameter specifies if the whole matrix is used, or an knn-graph. The default depends on the  mode - 'CPM':  None uses full graph, and - 'modularity':  None uses square root of the number of features. resolution_parameter : float, default=None, Only required for mode 'CPM'. If None, the resolution parameter will be set to the third quartile of  X for  n_neighbors=None and else to the mean value of the knn graph. seed : int, default=None, Use an integer to make the randomness of Leidenalg deterministic. By default uses a random seed if nothing is specified. Attributes      clusters_ : ndarray of shape (n_clusters) The result of the clustering process. A list of arrays, each containing all indices (features) for each cluster. matrix_ : ndarray of shape (n_features, n_features) Permuted matrix according to the found clusters. ticks_ : ndarray of shape (n_clusters) Get cumulative indices where new cluster starts in  matrix_ . permutation_ : ndarray of shape (n_features) Permutation of the input features (corresponds to flattened  clusters_ ). n_neighbors_ : int Only avaiable when using knn graph. Indicates the number of nearest neighbors used for constructin the knn-graph. resolution_param_ : float Only for mode 'CPM'. Indicates the resolution parameter used for the CPM based Leiden clustering. Examples     >>> import mosaic >>> mat = np.array( 1.0, 0.1, 0.9], [0.1, 1.0, 0.0], [0.8, 0.1, 1.0 ) >>> clust = mosaic.Clustering() >>> clust.fit(mat) >>> clust.matrix_ array( 1. , 0.9, 0.1], [0.8, 1. , 0.1], [0.1, 0. , 1.  ) >>> clust.clusters_ [array([0, 2]), array([1])] Initialize Clustering class."
},
{
